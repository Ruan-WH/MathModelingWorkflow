...
## CUMCM panel-label placement

When invoked through `3coding-visual`, place `(a)`, `(b)`, and subsequent panel labels horizontally centered below their corresponding axes. Reserve explicit bottom margin so labels do not collide with axis titles or notes. Do not use the usual top-left panel-label position for this workflow.
