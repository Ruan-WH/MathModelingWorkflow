...
## Math-modeling workflow bridge

When called by `3coding-visual`, this skill owns the figure contract, asset confirmation, panel architecture, typography, palette, export, and AP/CL/VI/VV quality checks. `3coding-visual` retains ownership of model semantics, real computed data, constraint definitions, convergence evidence, and reproducibility. Existing `.py` model workflows select Python without another backend question. Apply the CUMCM bilingual-font override in `references/typography.md` and the below-axis panel-label rule in `references/multipanel-layout.md`. For deterministic optimization, report seed/multistart behavior, discretization, feasibility margins, sensitivity, marginal contribution, or algorithm comparison instead of invented confidence intervals or significance tests.
