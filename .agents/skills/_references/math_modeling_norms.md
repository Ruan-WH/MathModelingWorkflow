...
- 优化模型在分项推导后应再用大括号或等价分组结构完整汇总，集中列出目标、变量域、状态耦合、资源或时序或空间约束、物理有效性和辅助定义；不能只写“约束见前文”，也不能为增加篇幅重复等价条件。
...
- 论文中的优化模型汇总式要能与求解器接口逐行对照；目标方向、变量类型、索引范围、硬约束、边界和辅助变量定义都应显式出现。公式较长时按约束类别分行或分组，保证不越出版心。
...
- 本项目的全国大学生数学建模竞赛中文摘要还必须执行 `../5writing/references/award-abstract-style.md`：采用“总述段 + 每问独立段 + 验证推广段”结构，问题段以加粗的“针对问题 X：”开头；保持 600--850 字主范围、5--9 处克制强调、三位小数口径和 4--6 个关键词，并在输出前完成一致性自检。
